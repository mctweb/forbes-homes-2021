<template>
  <div class="flex flex-wrap items-center w-full mb-3 bg-gray-400 group md:w-auto">
    <label for="appointment" class="flex items-center w-full px-4 pt-3 pb-2 text-sm text-gray-700 uppercase bg-gray-100 md:text-base" @click="checkboxClicked">Request Appointment
      <div class="flex flex-wrap items-center justify-end ml-auto">
        <div v-if="request && appointment" class="pl-2 ml-auto">{{ appointment }}</div>
        <IconCheckbox class="w-6 h-6 mb-1 ml-3 text-gray-500 hover:bg-white" :checked="request" />
      </div>
    </label>
